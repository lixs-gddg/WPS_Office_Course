...
void groupwindow::lineEditClicked()
{
...
void groupwindow::on_lineEdit_newtodo_returnPressed()
{
    ...
    newitem.item2=item;
    mytodoitem3.push_back(newitem);
    connect(checkBox,&QCheckBox::clicked, [=](){
        qDebug()<<"信号点击";
        auto i=mytodoitem3.begin();
        for(;i!=mytodoitem3.end();i++)
        {
            if(item==i->item2)
            {
                qDebug()<<i->label;
...
void groupwindow::on_listWidget_todolists_itemClicked(QListWidgetItem *item)
{
    bool flag=true;
    auto i=mytodoitem3.begin();
    for(indexnum3=0;i!=mytodoitem3.end();i++,indexnum3++)
    {
        if(item==i->item2)
        {
            qDebug()<<i->label;
...
void groupwindow::reflashall()
{
    delete  mytodoitem3[indexnum3].item2;
    QListWidgetItem *item=new QListWidgetItem;
    item->setSizeHint(QSize(1100, 80));
    ui->listWidget_todolists->addItem(item);
    QWidget *w=new QWidget;
    ...
    timelab->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    timelab->setStyleSheet("QLabel{font-size:15px;font-weight:15px;color:gray}");
    timelab->setText(mytodoitem3[indexnum3].time);
    repeatlab->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    repeatlab->setStyleSheet("QLabel{font-size:15px;font-weight:15px;color:gray}");
    switch(mytodoitem3[indexnum3].Repeat)
    {
    ...
    QPixmap pix;
    switch(mytodoitem3[indexnum3].level)
    {
    ...
    grouplab->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    grouplab->setStyleSheet("QLabel{font-size:15px;font-weight:15px;color:gray}");
    grouplab->setText(mytodoitem3[indexnum3].group);
    ...
    remindlab->setText(mytodoitem3[indexnum3].remind);
    fglab->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    fglab->setStyleSheet("QLabel{font-size:15px;font-weight:15px;color:gray}");
    fglab->setText("|");
    fglab2->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    ...
    fglab3->setText("|");
    lab->setAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    lab->setText(mytodoitem3[indexnum3].label);
    QCheckBox *checkBox=new QCheckBox(w);
    QGridLayout *layout=new QGridLayout;
    layout->setSpacing(10);
    if(mytodoitem3[indexnum3].level)
    {
        layout->addWidget(checkBox,0,0,1,1);
    ...
    if(mytodoitem3[indexnum3].remind=="未安排")
    {
        layout->addWidget(timelab,1,0,1,1);
        layout->addWidget(fglab,1,1,1,1);
        layout->addWidget(repeatlab,1,2,1,1);
    ...
    w->setLayout(layout);
    ui->listWidget_todolists->setItemWidget(item,w);
    mytodoitem3[indexnum3].item2=item;
    connect(checkBox,&QCheckBox::clicked, [=](){
        qDebug()<<"信号点击";
        auto i=mytodoitem3.begin();
        for(;i!=mytodoitem3.end();i++)
    ...
    updateitemtodb(mytodoitem3[indexnum3],1);
}


void groupwindow::on_comboBox_currentIndexChanged(int index)
{
    if(indexnum3<mytodoitem3.size()) mytodoitem3[indexnum3].level=3-index;
    reflashall();
}

void groupwindow::on_label_dtime_clicked()
{
...
void groupwindow::on_label_remindtime_clicked()
{
...
void groupwindow::reflashdtime(QDate dtime,int repeat)
{
    mytodoitem3[indexnum3].we=dtime.dayOfWeek();
    switch (mytodoitem3[indexnum3].we)
    {
    ...
    mytodoitem3[indexnum3].time=dtime.toString("yyyy年MM月dd日");
    mytodoitem3[indexnum3].Repeat=repeat;
    reflashall();
}

void groupwindow::reflashremindtime(QDate retime,QTime retimed)
{
    mytodoitem3[indexnum3].remind=retime.toString("yyyy年MM月dd日")+retimed.toString(" HH:mm")+" 提醒";
    ui->label_remindtime->setText(mytodoitem3[indexnum3].remind);
    reflashall();
}

void groupwindow::on_textEdit_desc_textChanged()
{
    mytodoitem3[indexnum3].description=ui->textEdit_desc->document()->toPlainText();
    updateitemtodb(mytodoitem3[indexnum3],1);
}

void groupwindow::on_textEdit_place_textChanged()
{
    mytodoitem3[indexnum3].place=ui->textEdit_place->document()->toPlainText();
    updateitemtodb(mytodoitem3[indexnum3],1);
}

void groupwindow::on_textEdit_note_textChanged()
{
    mytodoitem3[indexnum3].note=ui->textEdit_note->document()->toPlainText();
    updateitemtodb(mytodoitem3[indexnum3],1);
}

void groupwindow::reflashItems(todoitem &i)
{
    QListWidgetItem *item=new QListWidgetItem;
    item->setSizeHint(QSize(1100, 80));
    ui->listWidget_todolists->addItem(item);
...
void groupwindow::on_comboBox_2_currentIndexChanged(int index)
{
    for(auto i=mytodoitem3.begin();i!=mytodoitem3.end();i++)
    {
    ...
    case 0:
        for(auto i=mytodoitem3.begin();i!=mytodoitem3.end();i++)
            for(auto j=i;j!=mytodoitem3.end();j++)
            {
    ...
    indexnum3=0;
    qDebug()<<" ";
    for(auto i=mytodoitem3.begin();i!=mytodoitem3.end();i++,indexnum3++)
    {
        QListWidgetItem *item=new QListWidgetItem;
        item->setSizeHint(QSize(1100, 80));
        ui->listWidget_todolists->addItem(item);
        ...
        connect(checkBox,&QCheckBox::clicked, [=](){
            qDebug()<<"信号点击";
            auto i2=mytodoitem3.begin();
            for(;i2!=mytodoitem3.end();i2++)
            {
                if(item==i2->item2)
                {
                    qDebug()<<i2->label;
...
void groupwindow::on_pushButton_clicked()
{
    ...
    mytodoitem3.clear();
    QSqlQuery query1;
    QString sql=QString("select * from mtodolist3 where already=1 and groupname='%1';").arg(groupname);
    query1.exec(sql);
        while(query1.next())
        {
            todoitem newitem;
...
void groupwindow::additemtodb(todoitem i)
{
    QSqlQuery query2;
...
void groupwindow::updateitemtodb(todoitem i,int q)
{
    QSqlQuery query;
        //方式一，直接执行SQL语句
...
void groupwindow::on_pushButton_2_clicked(bool checked)
{
    if(checked)
    {
...
void groupwindow::addalreadyitem()
{
    for(auto i=alreadyitem3.begin();i!=alreadyitem3.end();i++)
    {
        if(i->item2!=nullptr) delete i->item2;
    }
    alreadyitem3.clear();
    QSqlQuery query1;
    QString sql=QString("select * from mtodolist3 where already=0 and groupname='%1';").arg(groupname);
    query1.exec(sql);
        while(query1.next())
        {
            todoitem newitem;
            ...
            alreadyitem3.push_back(newitem);
        }
}

void groupwindow::reflashalreadyitem(todoitem &i)
{
    QListWidgetItem *item=new QListWidgetItem;
    item->setSizeHint(QSize(1100, 80));
...
void groupwindow::on_listWidget_itemClicked(QListWidgetItem *item)
{
    auto i=alreadyitem3.begin();
    bool flag=true;
    for(;i!=alreadyitem3.end();i++)
    {
        if(i->item2==item)
        {
            flag=false;
